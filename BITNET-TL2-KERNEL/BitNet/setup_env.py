...
"""
==========================================================
    BitNet Setup Script — Function Execution Overview
==========================================================
...
----------------------------------------------------------
Purpose:
    Installs the GGUF Python package required to read/write
    and process GGUF model files.

Why Needed:
    BitNet and llama.cpp inference pipelines operate using
    GGUF format. Without this, model conversion and loading 
    is not possible.
...
Output:
    GGUF package installed in the current Python environment.

----------------------------------------------------------
🔹 STEP 2 — gen_code()
----------------------------------------------------------
Purpose:
    Generates LUT-based kernel code specifically optimized 
    for the BitNet model and CPU architecture.

Why Needed:
    BitNet performs AI inference without multiplication,
    using LUT (Lookup Table) kernels for faster execution 
    and reduced power usage.

Input:
    - Model name
    - CPU architecture (ARM / x86)
    - Quantization type (tl1 / tl2 / i2_s)

Output:
    - bitnet-lut-kernels.h
    - kernel_config.ini

These define optimized lookup-based compute operations.

----------------------------------------------------------
🔹 STEP 3 — compile()
----------------------------------------------------------
Purpose:
    Uses Clang + CMake to compile the generated kernel 
    and core engine source code into an executable.

Why Needed:
    The inference engine must be compiled before running 
    the optimized BitNet model locally.

Input:
    - Generated kernel header file
    - Source code files
    - System compilers

Output:
    Executable AI inference binary:

    Windows  → build/bin/Release/llama-cli.exe
    Linux/Mac → build/bin/llama

----------------------------------------------------------
🔹 STEP 4 — prepare_model()
----------------------------------------------------------
Purpose:
    Downloads (optional), converts, and quantizes the model
    to the GGUF format.

Why Needed:
    GGUF is optimized for hardware-efficient inference 
    and supports various quantization levels.
...
This is the final model loaded by the executable.
...
def gen_code():
    ...
    if arch == "arm64":
        if args.use_pretuned:
            pretuned_kernels = os.path.join("preset_kernels", model_name)
            if not os.path.exists(pretuned_kernels):
                logging.error(
                    f"Pretuned kernels not found for model {args.hf_repo}"
                )
                sys.exit(1)
...
def parse_args():
    ...
    # NEW: step selector (1–4) to run individually
    parser.add_argument(
        "--step",
        "-s",
        type=int,
        choices=[1, 2, 3, 4],
        help=(
            "Run a single step instead of full pipeline: "
            "1=setup_gguf, 2=gen_code, 3=compile, 4=prepare_model"
        ),
    )
...
if __name__ == "__main__":
    ...
    # If no step specified → original full behavior
    if args.step is None:
        logging.info("Running FULL pipeline: setup_gguf → gen_code → compile → prepare_model")
        main()
    else:
        # Individual step execution
        if args.step == 1:
            logging.info("Running STEP 1: setup_gguf()")
            setup_gguf()
        elif args.step == 2:
            logging.info("Running STEP 2: gen_code()")
            gen_code()
        elif args.step == 3:
            logging.info("Running STEP 3: compile()")
            compile()
        elif args.step == 4:
            logging.info("Running STEP 4: prepare_model()")
            prepare_model()
        logging.info(f"STEP {args.step} completed.")
